...
#define GRAFO_AVANZAR_NODO(nodo_apuntador,criterio_busqueda) \
		...
		case GRAFO_INDICE: \
			...
			break; \
		default: \
			perror("en GRAFO_AVANZAR_NODO %s hubo un error culero al buscar",__func__); \
			break; \
		}
...
typedef struct nodo {
	...
	nodo *siguiente = NULL;
	nodo *anterior = NULL;
	nodo *siguiente_indice = NULL;
	nodo *anterior_indice = NULL;
	nodo *siguiente_valor = NULL;
	nodo *anterior_valor = NULL;
	nodo *siguiente_distancia = NULL;
	nodo *anterior_distancia = NULL;
} nodo;

typedef struct grifo_contexto {
	int localidades_usadas = 0;
	nodo *inicio = NULL;
	nodo nodos_disponibles[MAX_NODOS];
	tipo_dato matrix_distancias[MAX_COLUMNAS_NODOS][MAX_FILAS_NODOS] = { {
			MAX_VALOR } };
	tipo_dato referencias_nodos_por_indice[MAX_FILAS_NODOS] = { 0 };
} grafo_contexto;
...
int init_grafo(tipo_dato[3][MAX_NODOS], grafo_contexto *ctx, bool
usar_hashes);
...
void init_zlog();
char *nodo_a_cadena(nodo *node, char *cadena_buffer);
